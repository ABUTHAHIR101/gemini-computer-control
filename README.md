...
│       ├── real_computer_controller.py  # 真实电脑控制器
│       └── background_controller.py  # 后台窗口控制器 (新增)
├── frontend/                    # 前端页面
│   ├── index.html              # 截图和控制界面
│   ├── playwright_sandbox.html # Playwright浏览器沙盒
│   ├── real_computer_sandbox.html # 真实电脑控制沙盒
│   └── background_sandbox.html # 后台窗口控制沙盒 (新增)
└── README.md                    # 项目说明
```
...
- 🌙 **后台窗口控制**: 使用 Win32 API 在后台操作窗口，不干扰用户操作 (新增)
...
后台控制功能使用 Win32 API 实现，可以在不干扰用户操作的情况下控制目标窗口。
...
支持的操作：
- `mouse_click`: 后台点击
- `mouse_double_click`: 后台双击
- `mouse_hover`: 后台移动鼠标
- `mouse_drag`: 后台拖动
- `mouse_scroll`: 后台滚轮
- `keyboard_type`: 后台输入文本
- `keyboard_press`: 后台按键
...
- **pywin32**: Windows API 调用（后台控制）
- **pywinauto**: Windows 自动化（可选）
...
后台控制模式使用 Win32 API 实现，可以在不干扰用户操作的情况下控制目标窗口。

### 特点

- ✅ **无需窗口焦点**: 目标窗口可以在后台或被遮挡
- ✅ **不干扰用户**: 物理鼠标不会移动，用户可以继续工作
- ✅ **多显示器支持**: 无论窗口在哪个屏幕都可以操作
- ✅ **后台截图**: 使用 PrintWindow API 截取被遮挡的窗口
...
2. 列出可用窗口：
```python
from backend.tools.background_controller import BackgroundController
...
3. 设置目标窗口：
```python
controller.find_window(title="记事本")
```
...
# 后台点击
controller.mouse_click(100, 200, "left")
...
1. **应用兼容性**: 某些现代应用（如 Chrome、Electron）可能不完全支持后台消息操作
2. **建议用途**: 传统 Win32 应用（如记事本、写字板）兼容性最好
3. **测试脚本**: 运行 `python test_background_control.py` 测试功能
